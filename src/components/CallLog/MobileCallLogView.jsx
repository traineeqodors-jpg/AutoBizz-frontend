  ...
  return (
   ...
   <div className="md:hidden flex flex-col gap-4">
           {processedLogs.map((log) => (
             <div key={log.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
               <div className="flex justify-between items-center">
                 <div className="flex items-center gap-3">
                    <div className="p-2 bg-gray-50 text-btn-100 rounded-xl"><IoCallOutline size={18}/></div>
                    <p className="font-bold text-text text-sm">{log.from}</p>
                 </div>
               ...
               <div className="grid grid-cols-2 gap-3">
                 <button onClick={() => setSelectedLog(log)} className="flex items-center justify-center gap-2 py-3 bg-gray-50 text-text font-bold rounded-xl text-xs border border-gray-100 active:scale-95 transition-all"><IoEyeSharp/> View</button>
